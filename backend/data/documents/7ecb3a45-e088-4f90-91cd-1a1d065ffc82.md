...
## RAG Flow (How Embeddings, Qdrant, and Chat Connect)
...
User Question
     ...
     ▼
┌─────────────────┐
│ Embed Question  │  ← Same embedding model used for documents
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│ Search Qdrant   │  ← Find similar document chunks by vector distance
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│ Retrieved Docs  │  ← Top K most relevant chunks
└────────┬────────┘
         │
         ▼
┌─────────────────────────────────────┐
│ LLM Prompt:                         │
│ "Given this context: [documents]    │
│  Answer this question: [user query]"│
└────────┬────────────────────────────┘
         │
         ▼
┌─────────────────┐
│ LLM Response    │  ← Answer grounded in your documents
└─────────────────┘
```
...
1. **Document Ingestion (one-time):**
   ...
   - Each chunk is converted to a vector embedding (e.g., 768 dimensions)
   - Vectors stored in Qdrant with original text as metadata

2. **Query Time:**
   - User question is embedded using the same model
   - Qdrant finds chunks with similar vectors (semantic search)
   - Retrieved chunks are injected into LLM prompt as context
   - LLM generates answer based on the provided context

3. **Key Points:**
   - Same embedding model must be used for documents and queries
   - Qdrant only compares vector distances, it does not understand text
   - LLM does not search, it only generates from provided context
...
### Phase 5: Frontend
- [D] React/Next.js chat widget
- [D] Message list with markdown rendering
- [D] Streaming response display
- [D] Session management
- [D] Mobile responsive design
- [D] Document management UI
  - [D] Document list page showing all uploaded documents
  - [D] Document upload form (admin only, requires API key)
  - [D] Upload progress indicator
  - [D] Document status display (pending/processing/ready/failed)
  - [D] Document view/download button in knowledge base
  - [D] Frontend API proxy routes for document upload and content
- [D] Chat history UI
  - [D] Backend: GET /api/chat/conversations endpoint to list user's past sessions
  - [D] Sidebar component showing list of past conversations
  - [D] Click to load/switch between conversations
  - [D] Conversation preview (first message or timestamp)
  - [D] New chat button to start fresh session

### Phase 6: Admin & Polish
- [D] Configurable ports via environment variables (BACKEND_PORT, FRONTEND_PORT)
- [D] ADMIN_API_KEY configurable via environment
